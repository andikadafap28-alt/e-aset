...
<form action="{{ route('aset.data.items') }}" method="GET" class="mb-4 flex flex-wrap items-center gap-3 p-3 bg-white rounded-lg border border-slate-200 shadow-sm">
    ...
    <select name="year" class="text-sm rounded-lg border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 py-1.5 pl-3 pr-8">
        <option value="">Semua Tahun</option>
        @foreach(range(date('Y') + 5, 2000) as $y)
            <option value="{{ $y }}" {{ request('year') == $y ? 'selected' : '' }}>{{ $y }}</option>
        @endforeach
    </select>
    <button type="submit" class="bg-blue-600 text-white px-4 py-1.5 rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors">Terapkan Filter</button>
</form>
