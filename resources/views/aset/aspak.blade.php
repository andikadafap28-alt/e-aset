...
@section('header_title', 'Data ASPAK')

@section('content')
<div class="mb-6 flex justify-between items-center" x-data>
    <div>
        <h2 class="text-2xl font-bold text-slate-800">Daftar Aset ASPAK</h2>
        <p class="text-slate-500 text-sm mt-1">Data Alat Kesehatan dari Kemenkes (ASPAK)</p>
    </div>
    <div class="flex flex-wrap gap-2">
        <button @click="$dispatch('open-modal-import')" class="bg-sky-600 hover:bg-sky-700 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-colors shadow-sm">
    ...
    <div x-data="{ showImportModal: false }" @open-modal-import.window="showImportModal = true" x-show="showImportModal" class="fixed inset-0 z-[100] flex items-center justify-center overflow-y-auto overflow-x-hidden bg-slate-900/50 backdrop-blur-sm" style="display: none;">
        <div @click.away="showImportModal = false" class="relative w-full max-w-md p-6 bg-white rounded-2xl shadow-xl transform transition-all">
            <div class="flex items-center justify-between mb-5">
                <h3 class="text-lg font-bold text-slate-900">Import File Excel ASPAK</h3>
            ...
            <form action="{{ route('aset.bmd.import') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="source" value="ASPAK">
                <div class="mb-5">
                    <label class="block text-sm font-medium text-slate-700 mb-2">Pilih File Excel ASPAK (.xlsx) <span class="text-rose-500">*</span></label>
                    <input type="file" name="file_excel" required accept=".xlsx, .xls, .csv" class="block w-full text-sm text-slate-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-sky-50 file:text-sky-700 hover:file:bg-sky-100 transition-colors cursor-pointer border border-slate-200 rounded-lg">
                </div>
                ...
                <div class="flex justify-end gap-3 mt-6">
                    ...
                    <button type="submit" class="bg-sky-600 hover:bg-sky-700 text-white text-sm font-medium py-2 px-5 rounded-lg shadow-sm transition-all flex items-center gap-2">Import Data</button>
                </div>
            </form>
        </div>
    </div>
